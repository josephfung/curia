...
const DELEGATE_MANIFEST: SkillManifest = {
  ...
  outputs: { response: 'string', agent: 'string', needs_clarification: 'boolean?', question: 'string?', partial_findings: 'string?', resume_token: 'string?' },
  permissions: [],
  secrets: [],
  timeout: 120000,
};
...
const REQUEST_CLARIFICATION_MANIFEST: SkillManifest = {
  ...
  action_risk: 'none',
  capabilities: [],
  inputs: { question: 'string', partial_findings: 'string' },
  outputs: { _curia_protocol: 'string', question: 'string', partial_findings: 'string' },
  permissions: [],
  secrets: [],
  timeout: 5000,
};
