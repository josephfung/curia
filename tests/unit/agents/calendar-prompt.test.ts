...
describe('calendar agent — key-loaded scheduling rules (ceo-inbox parity)', () => {
  ...
  it('bumps calendar agent version for scheduling-rules capability', async () => {
    ...
    // meaningful prompt/capability change. 0.7.4 = holds-sweep names scheduler-report +
    // negative list + expectedDurationSeconds + pin (#1831).
    expect(config.version).toBe('0.7.4');
  });
