...
describe('parseEmailMetadata', () => {
  ...
  it('returns safe defaults when metadata is undefined', () => {
    ...
    expect(meta.isAutoGenerated).toBe(false);
    expect(meta.autoGeneratedActionable).toBe(false);
    expect(meta.autoGeneratedSignals).toEqual([]);
    expect(meta.autoGeneratedCarveOuts).toEqual([]);
  });

  it('parses message-derived auto-generated fields (#1734)', () => {
    const meta = parseEmailMetadata({
      isAutoGenerated: true,
      autoGeneratedActionable: true,
      autoGeneratedSignals: ['auto_submitted', 'calendar_reply'],
      autoGeneratedCarveOuts: ['bounce'],
    });
    expect(meta.isAutoGenerated).toBe(true);
    expect(meta.autoGeneratedActionable).toBe(true);
    expect(meta.autoGeneratedSignals).toEqual(['auto_submitted', 'calendar_reply']);
    expect(meta.autoGeneratedCarveOuts).toEqual(['bounce']);
  });
