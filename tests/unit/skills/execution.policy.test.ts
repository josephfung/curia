...
import { TempFileStore } from '../../../src/skills/temp-file-store.js';
import type { ToolHandler, ToolManifest, ToolResult, ToolContext } from '../../../src/skills/types.js';
...
describe('autonomy gates', () => {
  ...
  describe('bypass ladder — woken/derived tasks', () => {
    ...
    // #1126: the elevated gate now requires a LIVE principal turn — a wake is NEVER live,
    // so a woken principal-LINEAGE task fails the gate at ANY score (this is the self-approval
    ...
    it('elevated gate rejects a derived child of a system task even at posture D (>= 90) — system never satisfies it', async () => {
      ...
      // satisfiable by system origination at all under the live-principal redefinition (#1126).
      const result = await new ExecutionLayer(registry, logger, { autonomyService: makeAutonomyService(90) })
        .invoke('approve-grant-recommendation', {}, undefined, {
    ...
    it('fails CLOSED on a woken task when the autonomy score is unavailable', async () => {
      ...
      const nullService = { getConfig: async () => null } as unknown as import('../../../src/autonomy/autonomy-service.js').AutonomyService;
  ...
  describe('Gate C — contact-tier gate', () => {
    ...
    function originatorMeta(
      tier: string,
      systemRole: string | null = null,
      extra?: { senderId?: string },
    ) {
      return {
        ...(extra?.senderId ? { senderId: extra.senderId } : {}),
        taskMetadata: {
          originator: {
    ...
    function makeEmailGateway(message: {
      from: Array<{ email: string }>;
      to?: Array<{ email: string }>;
      cc?: Array<{ email: string }>;
      fail?: boolean;
    }): OutboundGateway {
      const getEmailMessage = message.fail
        ? vi.fn().mockRejectedValue(new Error('nylas 404'))
      ...
      return { getEmailMessage } as unknown as OutboundGateway;
    }
    ...
    it('blocks low-risk KG write skill when initiated by blocked-tier contact (#1290)', async () => {
      const { judge, classifyAction } = makeEscalationJudge({ isThirdPartyFacing: true });
      const { registry, layer } = makeLayerWithScore100(undefined, judge);
      const handler = makeHandler('should not run');
      registry.register(makeRiskyManifest('memory-store', 'low'), handler);

      const result = await layer.invoke('memory-store', {}, undefined, originatorMeta('blocked'));
    ...
    it('allows low-risk KG write skill when initiated by known-tier contact (#1290)', async () => {
      const { registry, layer } = makeLayerWithScore100();
      const handler = makeHandler('ok');
      registry.register(makeRiskyManifest('memory-store', 'low'), handler);

      const result = await layer.invoke('memory-store', {}, undefined, originatorMeta('known'));
    ...
    it('clamps a judge class-downgrade up to the manifest floor (third-party send still escalates)', async () => {
      ...
      // Use a non-carveout skill so recipients stay unresolved and the judge still runs.
      ...
      const result = await layer.invoke('custom-notify', { to: 'stranger@example.com' }, undefined, originatorMeta('known'));
    ...
    it('honors a judge class-upgrade (irreversible escalates even for a reply-to-sender)', async () => {
      ...
      const result = await layer.invoke('custom-notify', { to: 'sender@example.com' }, undefined, originatorMeta('known'));
    ...
    it('fails closed when the judge returns no third-party determination', async () => {
      ...
      const result = await layer.invoke('email-send', {}, undefined, originatorMeta('known'));
    ...
    // -- email-reply recipient resolution (#1815): tier × reply-mode matrix ----

    it('allows known-tier email-reply in reply-all when the thread has no extra participants', async () => {
      const { judge, classifyAction } = makeEscalationJudge({ isThirdPartyFacing: true });
    ...
    it('rejects UUID-shaped reply_to_message_id even when Gate C would skip recipient resolution (#1817)', async () => {
      // Unknown tier → both reply-to-sender and third-party axes escalate, so the
      // decisionIfReplyToSender !== decisionIfThirdParty shortcut skips
      // resolveGateCRecipients. The shape check must still fire before that.
      const { judge, classifyAction } = makeEscalationJudge({ isThirdPartyFacing: true });
      const gateway = makeEmailGateway({ from: [{ email: 'alice@example.com' }] });
      const { registry, layer } = makeLayerWithScore100(undefined, judge, TEST_PRINCIPAL_IDENTITIES, {
        outboundGateway: gateway,
    ...
    it('escalates known-tier email-reply reply-all when CC contains anyone else', async () => {
      ...
      const result = await layer.invoke(
        'email-reply',
        { reply_to_message_id: 'msg-1', body: 'Looping everyone in' },
        undefined,
        originatorMeta('known', null, { senderId: 'alice@example.com' }),
      );
    ...
    it('allows known-tier email-reply with cc: "" (sender-only) even when the thread has others', async () => {
      const { judge, classifyAction } = makeEscalationJudge({ isThirdPartyFacing: true });
      const gateway = makeEmailGateway({
        from: [{ email: 'alice@example.com' }],
        to: [{ email: 'curia@example.com' }, { email: 'bob@example.com' }],
      ...
      const result = await layer.invoke(
        'email-reply',
        { reply_to_message_id: 'msg-1', body: 'Private reply', cc: '' },
        undefined,
        originatorMeta('known', null, { senderId: 'alice@example.com' }),
      );
    ...
    it('escalates known-tier email-reply when recipient resolution fails', async () => {
      const { judge, classifyAction } = makeEscalationJudge({ isThirdPartyFacing: false });
      const gateway = makeEmailGateway({ from: [{ email: 'alice@example.com' }], fail: true });
      const { registry, layer } = makeLayerWithScore100(undefined, judge, TEST_PRINCIPAL_IDENTITIES, {
        outboundGateway: gateway,
      });
      const handler = makeHandler('should not run');
      ...
      const result = await layer.invoke(
        'email-reply',
        { reply_to_message_id: 'missing', body: 'Thanks' },
        undefined,
        originatorMeta('known', null, { senderId: 'alice@example.com' }),
      );
    ...
    it('escalates unknown- and blocked-tier email-reply in every reply mode', async () => {
      const { judge, classifyAction } = makeEscalationJudge({ isThirdPartyFacing: false });
      const gateway = makeEmailGateway({ from: [{ email: 'alice@example.com' }] });
      const { registry, layer } = makeLayerWithScore100(undefined, judge, TEST_PRINCIPAL_IDENTITIES, {
        outboundGateway: gateway,
      ...
      for (const tier of ['unknown', 'blocked'] as const) {
        for (const input of [
          ...
          const result = await layer.invoke(
            ...
            input,
            undefined,
            originatorMeta(tier, null, { senderId: 'alice@example.com' }),
          );
          expect(result.success).toBe(false);
        }
      }
...
describe('humanApproved on InvokeOptions', () => {
  ...
  it('blocks elevated skills when task is system-originated (#1126: system no longer satisfies the gate)', async () => {
