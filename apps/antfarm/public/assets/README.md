...
served via `GET /api/antfarm/assets/*` only to an authenticated caller (a valid session cookie,
or the `x-web-bootstrap-secret` header for programmatic access). If a build omits that art,
