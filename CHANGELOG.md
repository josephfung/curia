...
- **`elevated` redefined as a live principal turn + all 19 elevated skills audited & reclassified** — `sensitivity: 'elevated'` now means "the current turn originated from a fresh principal inbound" (a dispatcher-stamped `livePrincipal` signal), enforced solely at the execution-layer gate (`isLivePrincipalTurn`); the duplicate handler-level origination re-checks on elevated skills were removed (the gate is the single enforcement point). System and woken/inherited principal-*lineage* contexts no longer satisfy it. CEO-authority primitives stay `elevated` (`approve`/`deny`/`dismiss-action`, `set-autonomy`, `approve`/`decline-grant-recommendation`); the rest moved to `normal` + `action_risk` so the autonomy engine governs them and they inherit the ADR-018 surface-and-confirm flow — `contact-merge` (medium), `contact-set-tier`/`contact-set-role` (high), `contact-grant-permission`/`contact-revoke-permission` (critical), plus `contact-update`/`contact-rename`/`behavioral-preferences-update`/`executive-profile-update`/`scheduler-create`/`scheduler-cancel`, and `list-pending-actions`/`web-browser` (now `normal` + `allowed_callers: [coordinator]`). The 8am digest still reads `list-pending-actions` because the coordinator is the caller, not because of system origination. **Public API note:** `sensitivity: 'elevated'` semantics changed (manifest schema surface). ADR-011/017/018 updated; specs 03/14. (#1126)
...
- **Self-approval hole closed** — redefining `elevated` as a *live* principal turn (#1126) means a woken/scheduled task carrying principal *lineage* can no longer approve, deny, or dismiss its own pending action, or change autonomy — exercising CEO authority now requires the CEO acting in the moment, with zero per-skill exceptions. The over-broad `system`-origination allowance (commit `3bd3d224`) that the gate carried is also removed; the read it existed for (`list-pending-actions`) is now gated on `allowed_callers` instead. The dispatcher strips any channel-supplied `livePrincipal` value before stamping its own, so the signal cannot be forged from an inbound message. (#1126)
