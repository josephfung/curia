...
- **`decay-warnings-list` skill** — read-only query that returns KG nodes currently in the "warned" state (flagged by DreamEngine before archival). Returns nodeId, label, nodeType, sensitivity, edgeCount, reason, daysRemaining, and warnedAt per entry, sorted oldest-first. Pinned to the coordinator so it can surface pending warnings to the CEO. (#280)
- **`memory-confirm` skill** — records the CEO's confirm-or-dismiss decision on a KG node flagged by the decay warning pass. `confirm` resets the decay clock (last_confirmed_at = NOW(), confidence = 1.0, cleared warned_at); `dismiss` archives immediately. Pinned to the coordinator. (#280)
