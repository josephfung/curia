...
- **Inbox triage** — observation-mode emails are now triaged with a 4-way protocol instead of being summarised. The dispatcher injects a structured preamble (URGENT / ACTIONABLE / NEEDS DRAFT / NOISE) into every observation-mode task, including the Nylas message ID and account so the coordinator can call tools directly. New `email-archive` skill archives a message via Nylas by removing its INBOX label (reversible). `OutboundGateway.archiveEmailMessage` added. Coordinator prompt updated with triage-aware guidance. Closes #296.

- **CEO inbox observation mode** (spec §03 / issue #273): `observation_mode: true` on a `channel_accounts.email` entry puts that adapter into observer mode — contact auto-creation is skipped, the contact trust flow and trust-score floor are bypassed in the dispatcher, and the coordinator receives the email with `observationMode: true` in metadata so it surfaces it as an observation rather than acting on it. `excluded_sender_emails` (supporting `env:VAR_NAME`) suppresses specified addresses from a monitored inbox to prevent self-reply loops. Coordinator prompt updated with observation mode behavior rules and inbox disambiguation. Closes the inbound portion of #273.

- **Skill discovery — callable discovered skills** (spec §03): discovered-but-not-pinned skills are now directly callable. After a successful `skill-registry` call, `AgentRuntime` expands the per-task working tool list with the full schemas of the returned skills via `ExecutionLayer.getToolDefinitions()`. The LLM can call any discovered skill in the same or subsequent turns with its real input schema — no proxy indirection, no schema loss. Expansion is per-task (concurrent tasks are isolated) and accumulates across multiple `skill-registry` calls. All discovered skills still route through the same elevation gate as pinned skills. Google Workspace MCP tools removed from coordinator `pinned_skills` — they are now discovered on demand. Closes #291.
