...
- **Drafts reachable in the CEO inbox** — `ceo-inbox-list`/`ceo-inbox-search` now query Nylas's `/drafts` resource for the DRAFTS folder instead of `/messages` (which never contains drafts), so existing drafts are listed and searchable by subject/recipient; results key on `drafts` to distinguish a genuine "none" from the old silent-zero. The inbox watermark is never applied to drafts. (#1000)
