...
- **Joseph's inbox observation mode** (spec §03 / issue #273): `observation_mode: true` on a `channel_accounts.email` entry puts that adapter into observer mode — contact auto-creation is skipped, the contact trust flow is bypassed in the dispatcher, and the coordinator receives the email with `observationMode: true` in metadata so it surfaces it as an observation rather than acting on it. `excluded_sender_emails` (supporting `env:VAR_NAME`) suppresses specified addresses from a monitored inbox to prevent self-reply loops. Coordinator prompt updated with inbox disambiguation ("my inbox" → Joseph's, "your inbox" → Curia's) and observation mode behavior rules. Closes the inbound portion of #273.
