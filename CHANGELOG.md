...
- **`knowledge-writing-config` skill** — Store and retrieve writing workflow configuration (Writing Guide URL, Essays Index URL, etc.) in the knowledge graph. Values use `permanent` decay class — stable URLs the CEO provides once via chat. Used by the essay-editor agent to locate Writing Guide and Essays Index without hardcoding URLs in YAML.
