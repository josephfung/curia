...
- **Calendar scheduling rules** — the calendar specialist loads CEO standing scheduling preferences from `config-store` (`calendar`/`rules`) by key at task start (ceo-inbox parity), with rules-management delegation and prompt-structure tests; standing prefs no longer rely on semantic `memory-query`. (#1223)
