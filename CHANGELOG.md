...
- **Secrets vault** — encrypted (AES-256-GCM) secrets storage in PostgreSQL, backed by a single `SECRET_ENCRYPTION_KEY` env var (auto-generated by setup). `ctx.secret()` reads from the vault first with an env-var fallback for incremental migration. Includes key rotation script and ADR-020. Closes #542.
...
- **OpenSSF Best Practices badge** — README now links to the project's CII Best Practices entry.
