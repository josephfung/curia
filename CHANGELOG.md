...
- **Elevated-skill gate + autonomy principal-bypass input** — both now consume *effective* (post-ladder) standing rather than raw lineage, so a heartbeat-woken task below its posture threshold no longer rides borrowed principal/system standing. The gate's requirement is unchanged in this step (principal-or-system); only its input changed. (#1125)
