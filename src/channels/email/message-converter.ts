...
import {
  ...
  type AutoGeneratedCarveOut,
  type AutoGeneratedSignal,
} from './auto-generated-mail.js';
...
export interface ConvertedEmail {
  ...
  metadata: {
    ...
    /**
     * Message-derived auto-generated indicator (#1734). Independent of the
     * sender contact's `kind` — a person's calendar client can emit machine mail.
     */
    isAutoGenerated: boolean;
    /** Signals that fired (headers, calendar method, OOO subject). */
    autoGeneratedSignals: AutoGeneratedSignal[];
    /**
     * True when a carve-out (bounce, payment failure, security alert, hard
     * deadline) requires escalation despite auto-generation.
     */
...
export function convertNylasMessage(msg: NylasMessage, selfEmail?: string): ConvertedEmail {
  ...
  // Message-derived auto-generated classification (#1734). Uses headers,
  // calendar attachment method (consuming isCalendarInvite), and body emptiness
  // — not the sender contact's kind.
  const autoGenerated = toAutoGeneratedMetadata(
    classifyAutoGeneratedMail({
      subject: msg.subject,
      bodyText,
      snippet: msg.snippet ?? '',
