...
export async function humanClick(page: Page, locator: Locator, opts: BehaviorOptions = {}): Promise<void> {
  ...
  // Custom-styled radios/checkboxes (e.g. 16personalities) hide the native input but keep
  // aria-label on it. Playwright resolves them, yet they fail actionability checks — force
  // click still toggles the control and emits the input events the site's JS listens for.
