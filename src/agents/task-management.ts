...
export const TASK_MANAGEMENT_BLOCK = [
  ...
  'conversation — only its title, `intent_anchor`, and progress note. Those must re-establish',
  'the work: the goal, the resource (e.g. a URL), and precise progress ("answered Q1-12 of',
  '60"). A bare title like "Answer Q13-36" strands the wake.',
  '',
  'If a subtask works through the **web-browser** skill, note that its live browser session',
  'does not survive a wake: record the URL and the exact `session_id` the tool returned, and',
  'on wake reuse it — or re-navigate and resume if the tool reports the session is gone.',
  '',
