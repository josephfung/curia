...
export interface EmailMetadata {
  ...
  /** Message-derived auto-generated flag (#1734). False when absent or non-boolean. */
  isAutoGenerated: boolean;
  /** True when an actionable carve-out requires escalation despite auto-generation. */
  autoGeneratedActionable: boolean;
  /** Signal names from the classifier; empty when absent. */
  autoGeneratedSignals: string[];
  /** Carve-out names from the classifier; empty when absent. */
  autoGeneratedCarveOuts: string[];
}
...
export function parseEmailMetadata(metadata: Record<string, unknown> | undefined): EmailMetadata {
  ...
  return {
    ...
    autoGeneratedActionable: raw?.autoGeneratedActionable === true,
    autoGeneratedSignals: Array.isArray(raw?.autoGeneratedSignals)
      ? (raw.autoGeneratedSignals as unknown[]).filter((s): s is string => typeof s === 'string')
      : [],
    autoGeneratedCarveOuts: Array.isArray(raw?.autoGeneratedCarveOuts)
      ? (raw.autoGeneratedCarveOuts as unknown[]).filter((s): s is string => typeof s === 'string')
      : [],
  };
}
