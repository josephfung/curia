...
export class Dispatcher {
  ...
  private async handleInbound(event: InboundMessageEvent): Promise<void> {
    ...
    if (payload.channelId === 'email') {
      ...
      // Non-actionable → suppress at dispatch (no agent.task, no outbound reply).
      // Actionable carve-out → still run coordinator with a silence+escalate preamble;
      // the relay is still suppressed so a narrated closing cannot email the sender.
      if (emailMeta.isAutoGenerated) {
        if (!emailMeta.autoGeneratedActionable) {
          this.logger.info(
            {
              channelId: payload.channelId,
              conversationId: payload.conversationId,
            ...
            'Dispatcher auto-generated mail: suppressing reply — no agent.task',
          );
          await this.bus.publish(
            'dispatch',
            createOutboundNoReply({
              // No agent.task was created; stamp the inbound event id so the
              // audit trail still has a stable routing key (#1734).
              routingTaskId: event.id,
              agentId: 'coordinator',
              conversationId: payload.conversationId,
              channelId: payload.channelId,
        ...
        taskContent = buildAutoGeneratedPreamble({
          signals: emailMeta.autoGeneratedSignals,
          carveOuts: emailMeta.autoGeneratedCarveOuts,
        }) + taskContent;
        this.logger.info(
          {
            channelId: payload.channelId,
          ...
          'Dispatcher auto-generated mail: actionable carve-out — dispatching with silence preamble',
        );
      }
    }
