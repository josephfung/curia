...
import { parseEmailMetadata, sanitizeNylasMessageId, buildCcPreamble, buildMessageIdBlock, buildThreadParticipantsBlock } from './email-metadata.js';
 ...
 * SECURITY: strips `ceoInitiated` (legacy), `originator`, `delegationOrigin`, and
 * `inboundNylasMessageId` after all untrusted spreads — so neither channel metadata nor
 * injection metadata can smuggle a forged originator, a forged reply-lock task id, or a
 * forged Nylas message id. The trusted `originatorMeta` spread wins last.
 * `delegationOrigin` is set only by the delegate skill on the specialist task, never here.
 * `inboundNylasMessageId` is the sanitized id stamped here for email inbounds (#1909).
...
export class Dispatcher {
  ...
  private async handleInbound(event: InboundMessageEvent): Promise<void> {
    ...
    // Sanitized Nylas message id for this inbound. Stamped into trusted metadata
    // so a specialist brief can carry it (#1909). Undefined for non-email
    // channels and when sanitization fails — never the raw value.
    ...
    if (payload.channelId === 'email') {
      ...
      const messageIdBlock = buildMessageIdBlock(inboundNylasMessageId);
      if (messageIdBlock !== null) {
        taskContent = messageIdBlock + taskContent;
      }
      ...
      // Layered on top of the shared Message ID line so the id is not duplicated.
      if (emailMeta.curiaRole === 'cc') {
        ...
        taskContent = buildCcPreamble(emailMeta, payload.accountId) + taskContent;
      }
