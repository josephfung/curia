...
export class EmailSendHandler implements ToolHandler {
  async execute(ctx: ToolContext): Promise<ToolResult> {
    ...
    const { to, cc, subject, body, reply_to_message_id: replyToMessageIdRaw, attachments: attachmentsRaw, context_bridge: contextBridgeRaw } = input;
    // Same parser as email-reply. The quote fetch and the send must use it
    // together or a secondary-mailbox thread is quoted from the primary (#1832).
    ...
    // Defense in depth (#1734): block email-send to the machine sender on an
    // auto-generated turn. Escalation to the principal (a different To) stays open.
    const autoGenBlock = autoGeneratedSenderBlockReason(ctx.taskMetadata, toAddresses[0]!);
    if (autoGenBlock) {
