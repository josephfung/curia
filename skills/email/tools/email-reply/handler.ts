...
export class EmailReplyHandler implements ToolHandler {
  async execute(ctx: ToolContext): Promise<ToolResult> {
    ...
    const { reply_to_message_id: replyToMessageId, body, cc: ccInput, attachments: attachmentsRaw, context_bridge: contextBridgeRaw } = input;
    // Shared with Gate C's resolveEmailReplyRecipients so both fetches use the
    // same cache key (#1832). Blank means the primary mailbox.
    ...
    // Defense in depth (#1817): Gate C's resolveEmailReplyRecipients also rejects
    // this shape before fetchMessage. Keep the check here so principal-bypass /
    // non-Gate-C paths still get the actionable error without a Nylas round-trip.
    ...
    ctx.log.info({ replyToMessageId, accountId }, 'Replying to email via gateway');

    try {
      const original = accountId === undefined
        ? await ctx.outboundGateway.getEmailMessage(replyToMessageId)
        : await ctx.outboundGateway.getEmailMessage(replyToMessageId, accountId);
