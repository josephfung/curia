...
- Outbound messages to a disconnected Signal channel are queued in Postgres (max 100 per channel, 24h TTL). When Signal reconnects, queued messages are delivered in order (#1380).
...
Outbound messages to a disconnected Signal channel are queued in Postgres (max 100 per channel, 24h TTL) and delivered in order on `channel.reconnect` (#1380).
