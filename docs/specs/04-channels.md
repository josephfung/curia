...
- **Outbound:** `OutboundGateway` → `chat.postMessage` (mrkdwn). Channel mentions and active-thread replies stay in-thread. Principal checks use Slack user id `U…` via `isPrincipalIdentity('slack', …)`, not conversation ids.
