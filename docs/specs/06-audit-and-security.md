...
| `outbound.no_reply` | dispatch | The agent declined to reply (`NO_REPLY` sentinel). No `outbound.message` is published. Reasons: `'agent_declined'` (normal turn) or `'content_block_abandoned'` (rewrite-path escape). Distinct from `outbound.suppressed_duplicate`. See [spec 05](05-error-recovery.md). (#1732) |
