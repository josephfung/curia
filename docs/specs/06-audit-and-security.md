...
| `outbound.no_reply` | dispatch | The agent declined to reply (`NO_REPLY` sentinel), returned empty content, produced a near-miss control token, or inbound mail was classified as auto-generated with no actionable carve-out. No live `outbound.message` is published. Reasons: `'agent_declined'`, `'content_block_abandoned'`, `'empty_response'`, `'ambiguous_decline'`, `'auto_generated'`. Distinct from `outbound.suppressed_duplicate`. See [spec 05](05-error-recovery.md). (#1732, #1734) |
