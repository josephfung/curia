...
- The curated licensed-art subtree lives in `curia-deploy` and is `COPY`-layered into `apps/antfarm/assets-licensed/` at build (runtime PNGs only — no `.ase`/`.gif`/`.txt` masters). A checkout without the subtree still builds; the open-core image simply has no licensed art and renders placeholders.
