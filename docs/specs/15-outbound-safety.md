...
2. **Explicit no-reply** (`outbound.no_reply`) — the agent's entire response is the sentinel
   ...
   independent of contact `kind`). Auto-generated turns also stamp
   `autoGeneratedSuppress` / `autoGeneratedSenderId` on task metadata so
   `email-reply` / `email-send` cannot mail the machine sender (principal
   escalation to a different recipient stays open).
...
- **Principal-tier bypass (#1225).** When the recipient tier is `principal`, Stage 2.5 short-circuits and returns no findings *before* any classification call — the policy table above already permits every disclosure class to the principal, so running the judge could only ever add a fail-closed false block. This is what stops a transient judge failure from blocking principal-bound mail such as the daily recap (mirroring Stage 2's sole-principal-recipient bypass). Non-principal recipients are unaffected: the judge still runs for them.
