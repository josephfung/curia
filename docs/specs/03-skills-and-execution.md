...
- `sensitivity`: `"normal"` or `"elevated"`. **`elevated` means the skill requires a *live principal turn*** — the current turn must have originated from a fresh principal inbound (the dispatcher stamps a `livePrincipal` signal on those, never on wakes, derivations, or scheduler fires). It is the live-principal authority primitive: only the CEO, acting now, may invoke it. System, agent, and woken/inherited principal-*lineage* contexts all fail, with zero per-skill exceptions — this is what closes the self-approval hole (a woken principal-lineage task cannot approve its own pending action). Enforced **solely** at the execution-layer gate (`isLivePrincipalTurn`); elevated skills carry no handler-level origination re-check (#1126). Consequential autonomous actions that are *not* CEO-authority primitives are `normal` + `action_risk` instead, so they are governed by the autonomy engine and inherit the ADR-018 approval flow. See ADR-017 and `docs/wip/2026-06-22-woken-task-authorization-design.md`.
...
3. **Validate elevation** — if `sensitivity: elevated`, reject unless this is a **live principal turn** (`isLivePrincipalTurn` on the effective metadata: the dispatcher-stamped `livePrincipal` signal plus principal lineage). System, agent, and woken/inherited principal-lineage contexts all fail — a wake is never a live turn (#1126). This is the sole enforcement point; no handler re-check duplicates it. (Distinct from the autonomy principal-bypass, which uses *lineage* via the ladder — see [14-autonomy-engine.md](14-autonomy-engine.md#effective-standing--the-bypass-ladder-wokenderived-tasks).)
