...
Publish rejection (not an agent failure — the bus swallows subscriber errors) reverts `running` → `pending` on the detached promise, but only for the generation it claimed: the `pendingJobs` entry for that task event, and the row only while `run_started_at` is still the timestamp the claim returned. The claim returns that timestamp as text. `pg` would otherwise parse it as a `Date` and drop microseconds, and the revert comparison would miss the row. A rejection that arrives after the slot timeout — once the watchdog has reset the row and a later poll has claimed it again — must not clear the newer run. Claim and payload errors before the handoff stay on the poll's own catch and are not generation-scoped, because no task event exists yet.
