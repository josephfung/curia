...
- For a **US** long code: brand + **Low-Volume Mixed** 10DLC campaign (Sole Proprietor
  only if no EIN). A **Canadian** office DID is a viable alternative outside 10DLC.

## Steps

1. In Telnyx Mission Control, complete 10DLC when using a US long code.
2. Buy a **voice+SMS** number (prefer voice+SMS so Phase 2 PSTN can reuse it).
...
Telnyx enforces carrier STOP (sends to a suppressed number fail with error **40300**).
Curia does **not** keep a parallel opt-out table. Inbound "STOP" / "don't text me"
messages reach the agent as ordinary SMS; the agent should record the preference as a
contact KG fact and treat a 40300 bounce as authoritative (do not retry).

## Cost floor

US steady-state is roughly **~$3–5/mo** after ~$20 one-time 10DLC registration
(number rent + Low-Volume Mixed + usage). International send is extra. Canadian
DIDs avoid 10DLC but have different regulatory/cost trade-offs.
...
- App-level keyword STOP/START/HELP auto-replies (carrier + agent KG facts instead)
