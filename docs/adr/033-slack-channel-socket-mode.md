...
- `inbound.reaction.conversationId` is best-effort (may key on a thread-reply ts rather than the thread root). Approval correlation uses `targetMessageId` ↔ `outbound.delivered.messageId` via `approval_message_bindings` (#1479), never `conversationId`.
...
- Reaction→approval is wired in #1479 (channel-agnostic mapper in autonomy/; Slack + Signal).
