# ADR-041: AWS Bedrock as the configured LLM provider

Date: 2026-07-06
Status: Proposed

> **Provenance.** Authored by [@AnshulaChowdhury](https://github.com/AnshulaChowdhury)
> in [#1349](https://github.com/josephfung/curia/pull/1349), which was not merged.
> The record is preserved here because the design and the empirical model-selection
> findings below stand on their own, independent of that PR's code. Renumbered from
> 023 (taken by ADR-023 in the interim) and re-statused from Accepted to Proposed:
> no Bedrock provider exists in `main`, so this documents an approach under
> consideration, not one in force. The body is otherwise unedited, including its
> references to `src/agents/llm/bedrock-mistral.ts` and other files that were never
> merged and do not exist in the tree. Implementation is tracked by #1766; the
> orphaned-turn bug noted in Consequences is tracked by #1767.
